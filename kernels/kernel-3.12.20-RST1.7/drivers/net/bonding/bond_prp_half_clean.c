...
//    return &common.node;
    ...
    volatile u32		 hash = 0;
    //volatile u8			 cmp = 0;
    u32 curTimeStampMS=0,deltaTimeStampMS=0;
    ...
    for( pn=NULL, hash=htonl( *((u32 *)(mac+2))), i=0; i<NumValidBitsInMAC; i++, hash >>= 1)
...
struct sk_buff *bondPRP_rx_frameTYPE_regular(struct sk_buff *skb, u16 ethFrmSIZE, u16 ethHdrSIZE)
{
    ...
    #endif
    volatile u32 curTimeStampMS=0;
    volatile u32 lastTimeStampMS=0;
    volatile u32 deltaTimeStampMS=0;
...
/*
	...
	if( skb_is_nonlinear(skb)) {
		...
		printk(KERN_ALERT "MAC non linear\n");	
		// TODO Tratare caz MAC fragmentat
		if (f->size < ETH_2_ALEN)	{
			printk("Error  frag mac ret=%d\n", ret);
		}
		else {
			u8 *vaddr = kmap_atomic(skb_frag_page(f));
			memcpy(dstmac, vaddr + f->page_offset + ETH_ALEN, ETH_ALEN);
			kunmap_atomic(vaddr);
			macptr = dstmac;
		}
	}
...
//                        printk(KERN_ALERT"ERROR: wrong PRPtrailerTYPE %02x%02x\n",pdata[4],pdata[5]);
	...
	// !!!!!!!!! jiffies NU asigura o precizie suficienta !!!!!!!!!!!!
	// !!!!!!!!! tre folosit un timer mai rapid si monotonic !!!!!!!!!
	// !!!!!!!!! do_gettimeoffday nu e potrivit, folisesc temporar !!!
	do_gettimeofday(&tv);
	curTimeStampMS = ((tv.tv_sec * 1000) + (tv.tv_usec / 1000));

	spin_lock(&pn->lock);
	lastTimeStampMS = pn->lastTimeStampForSeqNR[rcvSeqNR & PRP_DROP_WINDOW_MSK];
	pn->lastTimeStampForSeqNR[rcvSeqNR & PRP_DROP_WINDOW_MSK] = curTimeStampMS;
	spin_unlock(&pn->lock);
	...
	printk(KERN_ALERT "delta=%u window=%d\n", deltaTimeStampMS, PRP_DROP_WINDOW_DeltaMiliSec);
	if( deltaTimeStampMS < PRP_DROP_WINDOW_DeltaMiliSec)
	{
		incCnt_PerLan(RxFrmDrOnLan, prp->lanID);
		//printk(KERN_ALERT"%d %s: DROPPP\n",__LINE__,__FUNCTION__);
		goto out_drop;
	}
    ...
    out_ok:
		printk(KERN_ALERT "Out ok leaves lid=%x frmsize%x recvseq %x\n", lanID, prpFrmSIZE, rcvSeqNR);
		return skb;
    out_drop:
		printk(KERN_ALERT "Out DROP leaves lid=%x frmsize%x recvseq %x\n", lanID, prpFrmSIZE, rcvSeqNR);
		kfree_skb(skb);
		skb=NULL;
		return NULL;
}
...
struct sk_buff *bondPRP_rx(struct sk_buff *skb)
{
	...
	// 4 = PRP_ETHTYPEvlanHDR_SIZE
	for( ethHdrSize = ETH_HLEN, i=ETH_2_ALEN; i < ethFrmSize; i+=PRP_ETHTYPEvlanHDR_SIZE, 
																	ethHdrSize+=PRP_ETHTYPEvlanHDR_SIZE)
	{
		printk(KERN_ALERT "%s %d skblen %d datalen %d i %d \n", __FUNCTION__, __LINE__,skb->len, skb->data_len, i);
/*
...
for (j = 0; j < skb_shinfo(skb)->nr_frags; j++)
{
        ...
        start = end;
			...
			// TODO Tratare caz EtherType fragmentat
			if (f->size >2048)	{
				printk("Error  frag mac\n");
				break;
			}
			else {
				u8 *vaddr = kmap_atomic(skb_frag_page(f));
				memcpy(etherType, vaddr+f->page_offset, 2048);
				kunmap_atomic(vaddr);
			}
			...
			printk("Nr_frags=%d Len=%d MacH=%d\n",skb_shinfo(skb)->nr_frags, skb->len, skb->mac_header);
