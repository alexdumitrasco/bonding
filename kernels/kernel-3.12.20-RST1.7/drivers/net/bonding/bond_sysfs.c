...
static ssize_t bonding_show_prp_stats	(struct device *d,
					 struct device_attribute *attr,
					 const char *buf, size_t count)
{
    struct bonding *bond = to_bond(d);
    
    return bondPRP_sysfs_show_prp_stats(bond,buf);
}
static DEVICE_ATTR(prp_stats, S_IRUGO, bonding_show_prp_stats, NULL);
